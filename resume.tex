...
\textit{Freelance Engineering} \hfill December 2024 - March 2024
\begin{itemize}
    \item Developed a flask API that performs astronomy calculations given a moment in time.
    \item Utilized JPL's DE441 ephemeris for computing planet state vectors for a moment in time.
    \item Used spherical geometry to calculate the boundary of the visible portion of the Earth for an observer not on Earth.
    \item Utilized Lunar osculating orbital elements to find the position of the lunar ascending node.
    \item Used Skyfield library to assist in universal time keeping, reading JPL ephemerides, and finding planet subpoints.
    \item Negotiated contract terms with client to arrive at an agreed upon deliverable and specifications.
\end{itemize}
}
...
\begin{itemize}
    ...
    \item Used 64-bit integers to encode the piece locations on the chess board.
    \item Implemented effiecent move generation by pre-computing each piece's attacks at compile time.
    \item Created a recursive function to debug move generation by comparing the move tree to known online results.
    \item Developed the code base with intense focus on readability, maintainability, and adherence to SOLID principles.
    \item Maintain an extremely comprehensive test suite with coverage report generation using googletest and gcovr.
    \item Designed an isolated build environment and continuous integration suite using make, docker, and Github Actions.
\end{itemize}
}
